...
public class Level {
	...
	public void build() throws Exception {
		...
		if (lines.size() < 3) {
			throw new Exception("To small connectiosn count " + lines.size());
		}
		if (lines.size() != connections.size()) {
			throw new Exception("No all lines connected (lines/connections)="
					+ lines.size() + "/" + connections.size());
		}
